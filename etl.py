...
def process_song_file(cur: cursor, filepath: str) ->None:
    """open song file,process and load to database
    """
...
def main():
    conn = psycopg2.connect("host=127.0.0.1 dbname=sparkifydb user=postgres password=Oluw@gbeng@")
    cur = conn.cursor()

    process_data(cur, conn, filepath='data/song_data', func=process_song_file)
    process_data(cur, conn, filepath='data/log_data', func=process_log_file)

    ## clsoe all db resources
    cur.close()
    conn.close()
